...
\begin{enumerate}
    % TODO: Jin Yang to write this properly
    \item \textbf{Removal of stopwords.} Stopwords are removed from the text as they are commonly used in informal social-media text and do not carry much sentiment signal. While this discards potential signals towards rage or excitement which may be important for sentiment classification, our experiments indicate that the signal was largely inconsistent so the generalization gain outweighted the potential information loss. 
    \item \textbf{Lemmatization.} Lemmatization is the process of reducing words to their base form, e.g., ``running'' to ``run''. While this may help reduce the vocabulary space, our experiments indicate that the signal was largely inconsistent so the generalization gain outweighted the potential information loss. 
\end{enumerate}


This preprocessing design balances simplicity and effectiveness: normalization steps reduce lexical variation, while negation tagging preserves sentiment-bearing structure that standard bag-of-words representations often miss.
