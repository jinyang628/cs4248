...
In addition to tweet text, the dataset includes metadata such as country, age group, time of tweet, population, land area, and density. However, our exploratory analysis suggested that these metadata features are weak predictors of sentiment. In particular, sentiment distributions remained broadly balanced across countries, age groups, and time-of-tweet categories. As a result, we chose to focus primarily on the tweet text itself rather than these auxiliary variables.
...
The following pre-processing steps are then added to the Naive Bayes pipeline exclusively, which empirically improved feature consistency for sentiment classification while preserving key polarity cues. A possible justification for the performance improvement/degradation of each step is also provided:
...
For the classical baseline, we trained a Multinomial Naive Bayes classifier. Text was vectorized using a count-based representation. We performed hyperparameter search over the vectorizer and model settings, including the n-gram range, minimum document frequency, vocabulary size, and smoothing parameter alpha.

The best-performing configuration used a CountVectorizer with max features = 6500, ngram range = (1, 2), min df = 1, and a Multinomial Naive Bayes classifier with alpha = 1.0. This indicates that unigram and bigram features were sufficient, while more complex n-gram ranges did not yield better cross-validation performance in the saved notebook.
