...
Therefore, we chose to drop these auxiliary variables and focus primarily on the tweet text itself rather than 
...
As our goal is to identify preprocessing steps that bridges the gap between Naive Bayes, we consider and begin with preprocessing and feature engineering that would greatly address the limitation, improve interpretabilty 
and performance of Naive Bayes, that will be less beneficial and counter productive for advanced models like Bi-LSTM followed by RoBERTa.
...
\begin{enumerate}
    ...
    \item \textbf{Lowercasing.} Converting tokens to lowercase reduces vocabulary sparsity. However, since all caps tokens (e.g GOOD) may serve as a potential signal, we ...
    ...
    \item \textbf{Lemmatisation.} Lemmatisation, reduces the vocabulary size and my improve the frequency estimation for rare forms. However, the morphological form may carry sentiment relevant signals and removing them might affect performance. Therefore, we conducted experiments to evaluate its contribution.
    \item \textbf{Stopword Removal.} Removing Stopwords, reduces the vocabulary size but is widely known to risk performance as by removing critical words that are negative in nature (e.g `not', `should'nt'). Therefore, we curated a list of certain stopwords that are crucial and are not excluded during the removal process (e.g not, shouldn't etc). Ultimately, we evaluate its contribution.
    
    \item \textbf{Translation of emoticons.} Emoticons are translated to their text equivalent ((e.g :( -> `sad face' and :) -> `happy face')) to prevent it from getting regarded as OOV (especially since it might be fragmented by tokenisation). This provides it more sentiment signal for improved performance.
