...
\author{A0251997L, A02345678Y, A0252829W, A0252459X \\
  Group 28 \\
...
In the industry, there have been many models used to do sentiment analysis on tweets. Most of the top performing models use LSTM-type models, according to nlpprogress.com\footnote{The website used: \url{https://nlpprogress.com/english/sentiment_analysis.html}.}. 

As shown in this website, the best sentiment analysis model\footnote{The model used: \url{https://arxiv.org/pdf/1906.08237}.} had an accuracy of 96\%, using a self-attention setup. The only preprocessing done was just encoding relative positioning in the vector representation.
...
\subsection{Tweets Preprocessing by Palonimo et al.}

However, significant research has also been done in the industry on improving preprocessing methods.

In this paper\footnote{The paper by Palonimo et al.: \url{https://www.mdpi.com/2076-3417/12/17/8765}.} by Palonimo et al, where they showcased a Naive Bayes Classifer used on tweets, and tested various preprocessing pipelines to test the effectiveness of diifferent preprocessing pipelines. 

They have also provided a table of their accuracy scores they obtained with different preprocessing pipelines, as well as with no preprocessing, which allows for a good benchmark for our models.
...
The dataset consists of tweet texts labelled with one of three sentiment classes: negative, neutral, or positive. The raw training set contains 27,481 rows and the raw test set contains 4,815 rows. After removing rows with missing values in critical fields such as text and sentiment, 1 row was dropped from the training set and 1,281 rows were dropped from the test set, leaving 27,480 training rows and 3,534 test rows. Within the training set, 11117 rows have neutral sentiment, 8582 have positive sentiment and 7781 have negative sentiment. As the distribution of sentiment seems to be rather balanced with no significant minority, we chose not to apply class-balancing techniques as oversampling and undersampling incur their own tradeoffs.

In addition to tweet text, the dataset includes metadata such as country, age group, time of tweet, population, land area, and density. However, our exploratory analysis suggested that these metadata features are weak predictors of sentiment. Sentiment distributions remained broadly balanced across these categories so we chose to focus primarily on the tweet text itself rather than these auxiliary variables.
...
As our goal is to analyse how model architectures affect performance, we apply the same preprocessing pipeline to all three models.
...
\begin{enumerate}
    ...
    \item \textbf{Translation of emoticons.} Emoticons are translated to their corresponding text representation to reduce vocabulary sparsity.
    ...
    \item \textbf{Removal of URLs.} URLs are removed as they are not informative for sentiment classification and can introduce noise into the feature space.
...
The following pre-processing steps are removed from the preprocessing pipelines as they did not improve the performance of the models:
...
Exploratory data analysis showed that the sentiment labels were relatively balanced across the three classes, so we did not apply class-balancing techniques such as oversampling or undersampling. We also examined several linguistic properties of the tweets. The corpus contained substantial numbers of URLs, hashtags, emoticons, and elongated expressions, confirming that the dataset reflects the noisy and informal nature of social media text.

While the dataset contains additional metadata such as country, age group, and time of tweet, our modelling pipeline focuses exclusively on textual features. Therefore, we restrict our models to operate on tweet text.
...
We also implemented an LSTM-based classifier as an intermediate neural baseline between Naive Bayes and RoBERTa. The LSTM pipeline uses a custom vocabulary with minimum frequency filtering, padded mini-batches, trainable embeddings, and a bidirectional LSTM encoder followed by dropout and a linear classification layer.

The preprocessing for the LSTM includes lowercasing, lemmatization, URL removal, translation of hashtags and emoticons, and retention of stopwords. This setup is intended to preserve sentiment-bearing function words while reducing noise from informal tweet formatting.

Hyperparameter tuning was performed over embedding dimension, hidden dimension, dropout rate, batch size, and bidirectionality. The best hyperparameter setting recorded in the notebook was 
...
\hline
\end{tabular}
\caption{Best-performing LSTM hyperparameters}
\end{table}
