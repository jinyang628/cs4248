...
For the classical baseline, we trained a Multinomial Naive Bayes classifier. 
Text was vectorized using a count-based representation. 
We performed hyperparameter grid search using sklearn's RandomizedSearchCV and GridSearchCV functions over the vectorizer and model settings, including the n-gram range, minimum document frequency, vocabulary size, and smoothing parameter alpha. 
Starting from a wide range for each feature, We did randomized grid search a few times, and slowly zoomed in to test more precise values.
=======
...
>>>>>>> 4a03b3f95fc0ebe09064f13aed33b7e55ca49524
...
The Bi-LSTM is trained with GloVe a pre trained embedding and when through a grid search to identify the best set of hyperparamter. To reduce complexity and consistency, the best set of hyperparamter from the baseline of the Bi-LSTM is trained and subsequently used for all teh following ablatiosn to identify the contributionsof certain pre processing steps that were effective 
on the Naive Bayes model but not as much the Bi-LSTM. The motivation of this model is to show how as the number of interpretbale feature decrreases adn better deep and complex representation of linguistic features, such as with the long range dependencies of the LSTM with is activation fucntion and with GloVe, the model starts performing better and that traditional preprocessing steps can become
