...
We apply a lightweight but targeted preprocessing pipeline to improve feature consistency for sentiment classification while preserving key polarity cues. The pipeline consists of the following steps:

\begin{enumerate}
    \item \textbf{Removal of uninformative columns.} We removed the following columns from our analysis: ``Population -2020'', ``Land Area (Km²)'', ``Density (P/Km²)'', ``Country'', ``Age of User'', ``Time of Tweet'', ``selected\_text'' as they do not contribute meaningfully to improving the performance of sentiment classification. Note that the value present in ``selected\_text'' is a subset of the value present in ``text''. While the process of deriving ``selected\_text'' is not explicitly documented in the dataset, our assumption is that it is the most relevant part of the tweet that led to the classification result. Regardless, this column is not present in the test set and we chose to exclude it from our training.
    \item \textbf{Removal of rows with critical missing values.} We removed rows with critical missing values in the ``text'' and ``sentiment'' columns.
    \item \textbf{Lowercasing.} All tokens are converted to lowercase to reduce vocabulary sparsity (e.g., ``Good'' and ``good'' are mapped to the same feature).
    \item \textbf{Removal of numbers.} Numeric tokens are removed because they rarely carry stable sentiment signals in our corpus and can introduce noise into bag-of-words features.
    ...
    \item \textbf{Removal of special characters and punctuation.} Non-alphabetic symbols and punctuation marks are removed to reduce tokenization noise from informal social-media text.
\end{enumerate}
